...
function list() {
  return new Promise(resolve => {
    $.get(basic(`task_state`), async (err, resp, data) => {
      try {
        if (err) {
          console.log(`${$.name} API请求失败，请检查网路重试`)
        } else {
          if (safeGet(data)) {
            const result = JSON.parse(data);
			console.log(`618集卡任务列表：${JSON.stringify(result)}`)
          }
        }
      } catch (e) {
        $.logErr(e, resp)
...
//basic
function basic(type,body) {
  return {
    url: `https://xinruimz1-isv.isvjcloud.com/mlyjyapi/${type}`,
    body: body,
    headers: {
	  'Cookie': cookie,
	  'Source' : `02`,
...
function login() {
  return {
    ...
    headers: {
	  ...
	  'Referer' : `https://xinruimz1-isv.isvjcloud.com/mlyjy/logined_jd/`,
      'accept-language': `zh-cn`
    }
  }
}
