...
if(!$.isNode()&&cookieYouth.indexOf("#") ==-1){
 cookieArr.push($.getdata('youthheader_zq'));
    ...
    if($.isNode()){
    if (process.env.YOUTH_HEADER1 && process.env.YOUTH_HEADER1.indexOf('#') > -1) {
        cookieYouth = process.env.YOUTH_HEADER1.split('#');
    } else if (process.env.YOUTH_HEADER1 && process.env.YOUTH_HEADER1.indexOf('\n') > -1) {
        cookieYouth = process.env.YOUTH_HEADER1.split('\n');
    } else {
        cookieYouth = [process.env.YOUTH_HEADER1]
    };
