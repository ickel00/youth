...
if(!$.isNode()&&ygkchd.indexOf("\n") ==-1){
   ...
   ygkcurl = ygkcurl.split("\n")
    ...
    Object.keys(ygkchd).forEach((item) =>{
        if (ygkchd[item]) {
        ygkchdArr.push(ygkchd[item])
        }
    });
    Object.keys(ygkcurl).forEach((item) =>{
        if (ygkcurl[item]) {
            ygkcurlArr.push(ygkcurl[item])
        }
    });	
 console.log(` ============= 您共提供${ygkchdArr.length}个睡眠赚账号 =============`);
}

if (isygkcck = typeof $request !== 'undefined') {
   ygkcck();
   $.done()
};
